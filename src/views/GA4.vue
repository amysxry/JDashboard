    ...
    <main class="analytics-main">
        ...
        <template v-else>
          ...
          <div class="kpi-card">
            <div class="kpi-header">
              ...
              <InfoTooltip text="Total de visitas, incluyendo múltiples visitas del mismo usuario" />
            </div>
          ...
          <div class="kpi-card">
            <div class="kpi-header">
                ...
                <InfoTooltip text="Tiempo promedio que los usuarios pasan en tu sitio" />
            </div>
          ...
          <div class="kpi-card">
            <div class="kpi-header">
                ...
                <InfoTooltip text="Porcentaje de visitas que abandonaron tu sitio sin interactuar" />
            </div>
          ...
          <div class="kpi-card">
            <div class="kpi-header">
                ...
                <InfoTooltip text="Número promedio de páginas vistas durante cada sesión" />
            </div>
      ...
      <section class="charts-section">
        <div class="chart-card">
          <div class="chart-header">
            ...
            <InfoTooltip text="Evolución diaria del tráfico a tu sitio web" />
          </div>
      ...
      <section class="conversions-analysis-section">
        <div class="section-header">
          ...
          <InfoTooltip text="Rendimiento de las acciones clave de tu sitio web" />
        </div>
        <div class="conversions-grid">
            ...
            <div class="chart-card">
                <div class="chart-header"><h3 class="chart-title">Conversiones por Nombre</h3></div>
                <template v-if="isLoadingTable || fetchError"><div class="chart-placeholder">{{ fetchError ? 'Error al cargar la gráfica.' : 'Cargando gráfica...' }}</div></template>
                <template v-else-if="gaConversions.length === 0"><div class="no-data">No hay datos de conversiones para mostrar.</div></template>
                <template v-else><div class="chart-container"><Bar :key="chartKey" :data="conversionsChartData" :options="conversionsChartOptions" /></div></template>
            </div>
        </div>
      </section>
      ...
      <section class="pages-section card-style">
        <div class="section-header">
            ...
            <InfoTooltip text="Contenidos más populares de tu sitio web" />
        </div>
        <template v-if="isLoadingTable || fetchError"><div class="chart-placeholder">{{ fetchError ? 'Error al cargar las páginas.' : 'Cargando tabla...' }}</div></template>
        <template v-else-if="mostVisitedPages.length === 0"><div class="no-data">No hay datos de páginas visitadas para el rango seleccionado.</div></template>
        <template v-else><div class="overflow-x-auto"><table class="pages-table"><thead><tr><th>Página</th><th>Vistas</th><th>Tiempo Promedio</th></tr></thead><tbody><tr v-for="(page, index) in mostVisitedPages" :key="index"><td class="page-path">{{ page.path }}</td><td>{{ formatNumber(page.views) }}</td><td>{{ formatDuration(page.avgTime) }}</td></tr></tbody></table></div></template>
      </section>
...
import { RefreshCw, ArrowUpIcon, ArrowDownIcon, CheckCircle, TrendingUp, Target, Users, Clock, CalendarDays, HelpCircle } from 'lucide-vue-next';
import { Chart as ChartJS, CategoryScale, LinearScale, PointElement, LineElement, BarElement, ArcElement, Title, Tooltip, Legend, Filler } from 'chart.js';
import { Line, Bar, Doughnut } from 'vue-chartjs';
import InfoTooltip from '@/components/InfoTooltip.vue'; // <-- Importamos el nuevo componente

// El resto del SCRIPT permanece igual, no hay cambios en la lógica.
...
const conversionRate = computed(() => { if (kpiData.value.sessions === 0) return 0; return totalConversions.value / kpiData.value.sessions; });
const keyInsight = computed(() => { if (gaDailyMetrics.value.length === 0) return null; if (conversionRate.value > 0.05) { return { icon: Target, title: '¡Excelente Tasa de Conversión!', text: `Tu tasa de conversión es del ${formatPercentage(conversionRate.value)}, lo que indica un gran rendimiento. ¡Sigue así!` }; } if (channelChartData.value.labels.length > 0) { const topChannel = channelChartData.value.labels[0]; return { icon: Target, title: 'Tu Canal Principal', text: `La mayoría de tus usuarios están llegando a través de '${topChannel}'. Considera potenciar las campañas en este canal para maximizar resultados.` }; } return { icon: Users, title: 'Rendimiento General', text: `Durante este período, has alcanzado un total de ${formatNumber(kpiData.value.activeUsers)} usuarios a través de ${formatNumber(kpiData.value.sessions)} sesiones.` }; });
...
const conversionsChartData = computed(() => ({ labels: gaConversions.value.map(c => c.conversion_name), datasets: [{ label: 'Cantidad de Conversiones', data: gaConversions.value.map(c => c.total_conversions), backgroundColor: ['#92d000', '#fe7529', '#4F46E5', '#3B82F6', '#FBBF24', '#8B5CF6', '#EF4444'], borderColor: '#2a2a2a', borderWidth: 1, }] }));
...
const responsiveDoughnutChartOptions = computed(() => { const options = JSON.parse(JSON.stringify(doughnutChartOptions)); if (isMobile.value) { options.plugins.legend.position = 'bottom'; options.plugins.legend.labels.padding = 20; } else { options.plugins.legend.position = 'right'; options.plugins.legend.labels.padding = 10; } return options; });
...
const fetchGaDailyMetrics = async () => { if (!clientGaId.value) { gaDailyMetrics.value = []; return; } const { startDate, endDate } = getDateRange(timeRange.value); const { data, error } = await supabase.rpc('get_daily_analytics_data', { p_client_id: clientGaId.value, p_start_date: startDate, p_end_date: endDate }); if (error) throw error; gaDailyMetrics.value = data || []; };
...
const fetchMostVisitedPages = async () => { if (!clientGaId.value) { mostVisitedPages.value = []; return; } const { startDate, endDate } = getDateRange(timeRange.value); const { data, error } = await supabase.rpc('get_most_visited_pages_data', { p_client_id: clientGaId.value, p_start_date: startDate, p_end_date: endDate }); if (error) throw error; gaPageMetrics.value = data || []; mostVisitedPages.value = gaPageMetrics.value
    .filter(p => !p.page_path.includes('/wp-admin/') && !p.page_path.includes('/wp-login.php') && !p.page_path.includes('/admin/'))
    .map(p => ({ path: p.page_path, views: p.page_views, avgTime: p.avg_time_on_page })); };
const fetchGaConversions = async () => { if (!clientGaId.value) { gaConversions.value = []; return; } const { startDate, endDate } = getDateRange(timeRange.value); const { data, error } = await supabase.rpc('get_conversions_data', { p_client_id: clientGaId.value, p_start_date: startDate, p_end_date: endDate }); if (error) throw error; gaConversions.value = data || []; };
const refreshData = async () => { fetchError.value = null; isLoading.value = true; isLoadingChart.value = true; isLoadingTable.value = true; try { if (!clientGaId.value) await fetchClientInfo(); if (clientGaId.value) { await Promise.all([ fetchGaDailyMetrics(), fetchGaDeviceMetrics(), fetchGaChannelMetrics(), fetchMostVisitedPages(), fetchGaConversions() ]); } else { gaDailyMetrics.value = []; gaDeviceMetrics.value = []; gaChannelMetrics.value = []; gaPageMetrics.value = []; mostVisitedPages.value = []; gaConversions.value = []; fetchError.value = fetchError.value || 'No se pudo obtener el ID del cliente.'; } } catch (err: any) { console.error("Error en refreshData:", err); fetchError.value = `Error al cargar los datos: ${err.message}`; } finally { isLoading.value = false; isLoadingChart.value = false; isLoadingTable.value = false; chartKey.value++; lastUpdated.value = new Date(); } };
onMounted(async () => { checkScreenSize(); window.addEventListener('resize', checkScreenSize); await refreshData(); });
...
.kpis-section { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem; margin-bottom: 2rem; overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(146, 208, 0, 0.2) transparent; }
.kpis-section::-webkit-scrollbar { height: 6px; }
.kpis-section::-webkit-scrollbar-track { background: transparent; }
.kpis-section::-webkit-scrollbar-thumb { background: rgba(146, 208, 0, 0.2); border-radius: 3px; }
.kpis-section::-webkit-scrollbar-thumb:hover { background: rgba(146, 208, 0, 0.3); }
...
.charts-section { display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 1.5rem; margin-bottom: 2rem; overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(146, 208, 0, 0.2) transparent; }
.charts-section::-webkit-scrollbar { height: 6px; }
.charts-section::-webkit-scrollbar-track { background: transparent; }
.charts-section::-webkit-scrollbar-thumb { background: rgba(146, 208, 0, 0.2); border-radius: 3px; }
.charts-section::-webkit-scrollbar-thumb:hover { background: rgba(146, 208, 0, 0.3); }
...
.overflow-x-auto { overflow-x: auto; scrollbar-width: thin; scrollbar-color: rgba(146, 208, 0, 0.3) rgba(0, 0, 0, 0.1); }
.overflow-x-auto::-webkit-scrollbar { height: 8px; }
.overflow-x-auto::-webkit-scrollbar-track { background: rgba(0, 0, 0, 0.1); border-radius: 4px; }
.overflow-x-auto::-webkit-scrollbar-thumb { background: rgba(146, 208, 0, 0.3); border-radius: 4px; }
.overflow-x-auto::-webkit-scrollbar-thumb:hover { background: rgba(146, 208, 0, 0.5); }
...
/* SE ELIMINÓ LA CLASE info-tooltip que ahora está en su propio componente */
...
@media (max-width: 820px) {
    ...
    .page-controls-header { justify-content: center; }
}
...
@media (max-width: 767px) {
  ...
  .page-controls-header { flex-direction: column; align-items: stretch; }
  .header-actions { flex-direction: column; gap: 0.75rem; }
  .time-select, .refresh-btn { width: 100%; }
}
